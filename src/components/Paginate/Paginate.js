...
export const Paginate = ({ itemsPerPage, slug, filterState }) => {
  ...
  useEffect(() => {
    ...
    dispatch(getCategoryCurrent(slug, itemOffset, filterState));
    const endOffset = itemOffset + itemsPerPage;
    setPageCount(Math.ceil(allFilmsLength / itemsPerPage));

  }, [itemOffset, allFilmsLength, slug]);
  ...
  const handlePageClick = (event) => {
    const newOffset = (event.selected * itemsPerPage) % allFilmsLength;
